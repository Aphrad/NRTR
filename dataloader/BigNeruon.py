...
def detection_collate(batch):
    ...
    return dict(image=torch.stack(img, 0), swc_target=swc_target, mat_target=mat_target)
...
class TrainDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return dict(image=torch.tensor(image.astype(np.float32)), swc_target=swc_target)
